...
object ManPage {
  ...
  abstract class Document {
    ...
    var title: String = ""
    var author: String = ""
    var date: String = autoGenerateDate()
    var version: String = scala.util.Properties.scalaPropOrElse("version.number", "(unknown)")
    var category: Value = USER_COMMANDS
    var encoding: String = "iso-8859-1"
    var sections: List[Section] = Nil
  }

  private def autoGenerateDate(): String = {
    import java.util.{Calendar, TimeZone, Locale}
    import java.text.SimpleDateFormat

    new SimpleDateFormat("yyyy-MM-dd").format(Calendar.getInstance(TimeZone.getTimeZone("UTC"), Locale.US).getTime)
  }
}
